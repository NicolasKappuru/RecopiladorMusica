...
string ArtistaDAOImpl::leerDato(int eleccion) {
    ...
    if (archivo.is_open()) {
        archivo.seekg((eleccion-1) * 99, ios::beg);  // Tama�o de la cadena +2
...
void ArtistaDAOImpl::modificarDato(int eleccion, string nuevoDato) {
    ...
    if (archivo.is_open()) {
        ...
        archivo.seekp((eleccion - 1) * 99, ios::beg);  // Posicionarse al inicio de la l�nea (+2 incluye salto de l�nea)
...
void ArtistaDAOImpl::eliminarNposLibre(int eleccion) {//Eliminara y pondra pos libre
	string posLibre;
	for (int i = 0; i < 97; ++i) { //Tama�o de la cadena
        posLibre += '0';  
    }
